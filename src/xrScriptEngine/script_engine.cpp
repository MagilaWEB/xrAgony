...
local this = {} \
%s this %s \
setmetatable(this, {__index = " LUA_GLOBAL "}) \
setfenv(1, this) ";
...
xr_vector<luabind::functor<bool>> UniqueCall;
...
{
	int l_iErrorCode;
	if (caNameSpaceName && xr_strcmp(LUA_GLOBAL, caNameSpaceName))
	{
...
bool CScriptEngine::function_object(LPCSTR function_to_call, luabind::object& object, int type)
{
	...
	if (xr_strcmp(name_space, LUA_GLOBAL))
	{
...
void CScriptEngine::parse_script_namespace(const char* name, char* ns, u32 nsSize, char* func, u32 funcSize)
{
	...
	if (!p)
	{
		xr_strcpy(ns, nsSize, LUA_GLOBAL);
		p = name - 1;
	}
	else
	{

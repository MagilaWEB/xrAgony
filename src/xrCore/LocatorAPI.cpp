...
bool CLocatorAPI::win_dir_delete(pcstr initial, pcstr nm)
{
	string_path fpath = "";
	if (initial && initial[0])
		update_path(fpath, initial, nm);
	...
	if (fileWinApi::is_directory(fpath))
	{
		fileWinApi::remove_directory(fpath);
		return true;
	}
...
void CLocatorAPI::win_file_delete(pcstr path, pcstr nm)
{
	string_path fname;
	if (path && path[0])
		update_path(fname, path, nm);

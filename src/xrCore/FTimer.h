...
public:
	...
	{
		const Duration current = super::getElapsedTime();
	...
	virtual Duration getElapsedTime() const
	{
		return getElapsedTime(super::getElapsedTime());
	}
};
...
#define CHECK_TIME(text, code) { clock_t tm_start = clock(); code \
clock_t ms = clock() - tm_start; Msg("* %s [%d ms], [%f sec]", text, ms, (float)ms / 1000); }

...
void CSheduler::DumpStatistics(IGameFont& font, IPerformanceAlert* alert)
{
	...
	font.OutNext("- update:	 %2.5fms, %2.1f%%", stats.Update.result, percentage);
...
void CSheduler::ProcessStep()
{
	...
	for (int i = 0; !Items.empty() && Top().dwTimeForExecute < dwTime; ++i)
	{
		...
		const u32 dwMax = (1000 + item.Object->GetSchedulerData().t_max) / 2;
		const float scale = item.Object->shedule_Scale();
		u32 dwUpdate = dwMinDelay + iFloor(float(dwMax - dwMinDelay) * scale);
		clamp(dwUpdate, u32(_max(dwMinDelay, u32(20))), dwMax);
		...
		{
#ifdef DEBUG_SCHEDULER
			Msg("SCHEDULER: process unregister (self unregistering) [%s][%x][%s]", item.scheduled_name.c_str(),
				item.Object, "false");
#endif
			continue;
		}

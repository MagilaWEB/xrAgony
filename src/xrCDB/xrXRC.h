...
public:
	...
	IC void ray_query(const u32 ray_mode, const CDB::MODEL* m_def, const Fvector& r_start, const Fvector& r_dir, float r_range = 10000.f)
	{
		Stats.RayQuery.Begin();
		CL.ray_query(ray_mode, m_def, r_start, r_dir, r_range);
		Stats.RayQuery.End();
	}

	IC void box_query(const u32 box_mode, const CDB::MODEL* m_def, const Fvector& b_center, const Fvector& b_dim)
	{
		Stats.BoxQuery.Begin();
		CL.box_query(box_mode, m_def, b_center, b_dim);
		Stats.BoxQuery.End();
	}

	IC void frustum_query(const u32 frustum_mode, const CDB::MODEL* m_def, const CFrustum& F)
	{
		Stats.FrustumQuery.Begin();
		CL.frustum_query(frustum_mode, m_def, F);

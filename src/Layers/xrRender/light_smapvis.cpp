...
void smapvis::invalidate()
{
	...
	testQ_V = 0;
	frame_sleep = ::IDevice->getFrame() + ps_r__LightSleepFrames;
	invisible.clear();
}
void smapvis::begin()
{
...
void smapvis::end()
{
	...
	case state_working:
		...
		if (testQ_V)
		{
			...
			testQ_frame = ::IDevice->getFrame() + 1; // get result on next frame
		}
		break;
	case state_usingTC:
		// nothing to do
...
{
	// the tough part
	if (testQ_frame != ::IDevice->getFrame())
		return;
...
{
	if (testQ_frame == (::IDevice->getFrame() + 1))
		testQ_frame--;
	flushoccq();
}

...
void CHW::CreateDevice(HWND m_hWnd, bool move_window)
{
	...
	HRESULT result = HW.pD3D->CreateDevice(DevAdapter, m_DriverType, m_hWnd,
	...
	CHECK_OR_EXIT(D3DERR_DEVICELOST != result, "Failed to initialize graphics hardware.\nPlease try to restart the game.\nCreateDevice returned D3DERR_DEVICELOST");
...
void CHW::Reset(HWND hwnd)
{
	...
	while (true)
	{
		...
		if (iter++ > 10)
			FATAL("! ERROR: [%dx%d]: %s", DevPP.BackBufferWidth, DevPP.BackBufferHeight);
...
void CHW::updateWindowProps(HWND m_hWnd)
{
	...
	if (bWindowed)
	{
		if (m_move_window)
		{
			bool bBordersMode = state_screen_mode != 3;

...
void dxEnvironmentRender::OnFrame(CEnvironment& env)
{
	...
	if (::Render->get_generation() == IRender::GENERATION_R2)
	{
		...
		{
			// tonemapping in VS
			mixRen.sky_r_textures.emplace_back(u32(CTexture::rstVertex), tonemap); //. hack
			mixRen.sky_r_textures_env.emplace_back(u32(CTexture::rstVertex), tonemap); //. hack
			mixRen.clouds_r_textures.emplace_back(u32(CTexture::rstVertex), tonemap); //. hack
		}
		else
		{
			// tonemapping in PS
			mixRen.sky_r_textures.emplace_back(2, tonemap); //. hack
			mixRen.sky_r_textures_env.emplace_back(2, tonemap); //. hack
			mixRen.clouds_r_textures.emplace_back(2, tonemap); //. hack
		}
	}
...
void dxEnvironmentRender::RenderSky(CEnvironment& env)
{
	...
	mSky.translate_over(::IDevice->cast()->vCameraPosition);
...
void dxEnvironmentRender::RenderClouds(CEnvironment& env)
{
	...
	mXFORM.translate_over(::IDevice->cast()->vCameraPosition);

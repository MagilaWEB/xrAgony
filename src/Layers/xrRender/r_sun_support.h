#ifndef r_sun_support_included
#define r_sun_support_included
#pragma once

constexpr u32 LIGHT_CUBOIDSIDEPOLYS_COUNT = 4;
constexpr u32 VIEW_FRUSTUM_RAYS_COUNT = 4;
...
public:
	...
	sun::ray view_frustum_rays[VIEW_FRUSTUM_RAYS_COUNT];
	sun::ray view_ray;
	sun::ray light_ray;
...
public:
		...
		xr_vector<Fplane>& dest, Fvector3& translation, float map_size, bool clip_by_view_near)
	{
		...
		for (u32 p = 0; p < align_planes_count; ++p)
		{
			float max_mag = 0;
			for (u32 i = 0; i < VIEW_FRUSTUM_RAYS_COUNT; ++i)
			{
	...
	bool check_cull_plane_valid(Fplane const& plane, float& sign, float mad_factor = 0.f)
	{
		...
		bool oriented = false;
		float orient = 0;
		for (u32 j = 0; j < VIEW_FRUSTUM_RAYS_COUNT; ++j)
		{
			float tmp_dist = 0.f;

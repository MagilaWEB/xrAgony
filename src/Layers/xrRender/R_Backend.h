...
#include "Layers/xrRenderDX11/r_backend_lod.h"
...
public:
	...
	void get_ConstantDirect(shared_str& n, u32 DataSize, void** pVData, void** pGData, void** pPData);

	// API
	void set_xform(u32 ID, const Fmatrix& M);
	...
	void set_PS(ID3DPixelShader* _ps, LPCSTR _n = nullptr);
	ICF void set_PS(ref_ps& _ps) { set_PS(_ps->sh, _ps->cName.c_str()); }

	void set_GS(ID3DGeometryShader* _gs, LPCSTR _n = nullptr);
	ICF void set_GS(ref_gs& _gs) { set_GS(_gs->sh, _gs->cName.c_str()); }

	void set_HS(ID3D11HullShader* _hs, LPCSTR _n = nullptr);
	ICF void set_HS(ref_hs& _hs) { set_HS(_hs->sh, _hs->cName.c_str()); }

	void set_DS(ID3D11DomainShader* _ds, LPCSTR _n = nullptr);
	ICF void set_DS(ref_ds& _ds) { set_DS(_ds->sh, _ds->cName.c_str()); }

	void set_CS(ID3D11ComputeShader* _cs, LPCSTR _n = nullptr);
...
protected: //	In DX10 we need input shader signature which is stored in ref_vs
	void set_VS(ID3DVertexShader* _vs, LPCSTR _n = nullptr);
public:
	...
	void set_Stencil(u32 _enable, u32 _func = D3DCMP_ALWAYS, u32 _ref = 0x00, u32 _mask = 0x00,
	...
	void dbg_Draw(D3DPRIMITIVETYPE T, FVF::L* pVerts, size_t vcnt, u16* pIdx, int pcnt);
	void dbg_Draw(D3DPRIMITIVETYPE T, FVF::L* pVerts, size_t pcnt);

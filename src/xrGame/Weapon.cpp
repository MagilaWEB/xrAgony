...
bool CWeapon::Action(u16 cmd, u32 flags)
{
	...
	case kWPN_FIRE:
	{
		// You can only ban it if the status is not shooting.
		// Otherwise, the status freezes on shooting.
		if (GetState() != eFire)
		{
			if (IsPending())
				return false;
...
void CWeapon::UpdateHudAdditonal(Fmatrix& trans)
{
	...
	if (Device.m_ScopeVP.IsSVPActive())
	{
		...
		Fvector hud_mHPB;
		trans.getHPB(hud_mHPB);

		//Компенсировать наклоны камеры при наклоне худа рук.
		Cam.setHPB
		(
			cam_mHPB.x,
			cam_mHPB.y,

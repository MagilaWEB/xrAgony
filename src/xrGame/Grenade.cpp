...
void CGrenade::State(u32 state, u32 old_state)
{
	...
	case eThrowEnd:
	{
		if (m_thrown)
		{
			...
			{
#ifdef MASTER
				Msg("Destroying local grenade[%d][%d]", ID(), ::IDevice->getFrame());
#endif // #ifdef MASTER
				DestroyObject();
			}
		};
...
void CGrenade::SendHiddenItem()
{
	if (GetState() == eThrow)
	{
		//		Msg("MotionMarks !!![%d][%d]", ID(), ::IDevice->getFrame());
		Throw();
	}
...
void CGrenade::PutNextToSlot()
{
	...
	if (m_pInventory)
	{
		...
		Msg("! PutNextToSlot : m_pInventory = nullptr [%d][%d]", ID(), ::IDevice->getFrame());

...
void CCameraLook::Move(int cmd, float val, float factor)
{
	switch (cmd)
	{
	case kCAM_ZOOM_IN: dist -= val ? val : (rot_speed.z * ::IDevice->TimeDelta_sec()); break;
	case kCAM_ZOOM_OUT: dist += val ? val : (rot_speed.z * ::IDevice->TimeDelta_sec()); break;
	case kDOWN: pitch -= val ? val : (rot_speed.x * ::IDevice->TimeDelta_sec() / factor); break;
	case kUP: pitch += val ? val : (rot_speed.x * ::IDevice->TimeDelta_sec() / factor); break;
	case kLEFT: yaw -= val ? val : (rot_speed.y * ::IDevice->TimeDelta_sec() / factor); break;
	case kRIGHT: yaw += val ? val : (rot_speed.y * ::IDevice->TimeDelta_sec() / factor); break;
	}
	if (bClampYaw)
...
void CCameraLook2::UpdateAutoAim()
{
	...
	yaw = angle_inertion_var(yaw, xyz.y, m_autoaim_inertion_yaw.x, m_autoaim_inertion_yaw.y, PI, ::IDevice->TimeDelta_sec());

	pitch =
		angle_inertion_var(pitch, xyz.x, m_autoaim_inertion_pitch.x, m_autoaim_inertion_pitch.y, PI, ::IDevice->TimeDelta_sec());
}
...
{
	Fquaternion new_dir;
	new_dir.slerp(m_current_dir, m_final_dir, ::IDevice->TimeDelta_sec()); // 1 sec

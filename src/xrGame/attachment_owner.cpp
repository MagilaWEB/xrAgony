...
void __stdcall AttachmentCallback(IKinematics* tpKinematics)
{
	...
	if (game_object->getDestroy()) return;
	...
	if (game_object->getVisible() && game_object->cast_actor())
	{
		for (; I != E; ++I) {
			Fmatrix att_mtx; kinematics->Bone_GetAnimPos(att_mtx, (*I)->bone_id(), u8(-1), false);

			(*I)->item().object().XFORM().mul(att_mtx, (*I)->offset());

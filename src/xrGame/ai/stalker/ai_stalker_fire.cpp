...
#pragma todo("Dima to Dima : forward-back bone impulse direction has been determined incorrectly!")
	...
	if (g_Alive() && (!m_hit_callback || m_hit_callback(&HDS)))
	{
		...
		if (::ScriptEngine->function_event<bool>("_G.CAI_Stalker__BeforeHitCallback", this->lua_game_object(), &tLuaHit, HDS.boneID))
			return;
...
void CAI_Stalker::update_best_item_info_impl()
{
	...
	{
		CScriptGameObject* GO = funct(this->lua_game_object(), m_item_actuality && m_best_item_to_kill ? m_best_item_to_kill->cast_game_object()->lua_game_object() : nullptr);
		if (GO)
		{

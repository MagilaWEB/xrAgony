...
bool CAI_Stalker::bfAssignObject(CScriptEntityAction* tpEntityAction)
{
	...
	case eObjectActionReload1:
	{
		...
		//			}
		//			else
		//				::ScriptEngine->script_log(LuaMessageType::Error, "cannot reload active item because it is
		// not
		// selected!");
		break;
	}
	...
	case eObjectActionTake:
	{
		...
		{
			::ScriptEngine->script_log(LuaMessageType::Error, "item is already in the inventory!");
			return ((l_tObjectAction.m_bCompleted = true) == false);
		}
	...
	case eObjectActionDrop:
	{
		if (!inventory().GetItemFromInventory(*l_tObjectAction.m_tpObject->cName()))
		{
			::ScriptEngine->script_log(LuaMessageType::Error, "item is not in the inventory!");
			return ((l_tObjectAction.m_bCompleted = true) == false);
		}

...
void CAI_Bloodsucker::set_visibility_state(visibility_t new_state)
{
	...
	if (::IDevice->TimeGlobal_ms() < m_visibility_state_last_changed_time + get_visibility_state_change_min_delay())
	{
		return;
	}

	m_visibility_state_last_changed_time = ::IDevice->TimeGlobal_ms();
...
void CAI_Bloodsucker::update_invisibility()
{
	...
	else if (::IDevice->TimeGlobal_ms() < m_runaway_invisible_time + default_runaway_invisible_time / 6)
	{
		set_visibility_state(partial_visibility);
	}
	else if (::IDevice->TimeGlobal_ms() < m_runaway_invisible_time + default_runaway_invisible_time)
	{

...
bool CTelekineticObject::init(CTelekinesis* tele, CPhysicsShellHolder* obj, float s, float h, u32 ttk, bool rot)
{
	...
	time_fire_started = 0;
	// time_raise_started	= ::IDevice->TimeGlobal_ms();
...
{
	// time_keep_started	= ::IDevice->TimeGlobal_ms();
	// state				= TS_Keep;
...
{
	if (time_keep_started + time_to_keep < ::IDevice->TimeGlobal_ms())
		return true;
	return false;
}
...
{
	if (time_fire_started + FIRE_TIME < ::IDevice->TimeGlobal_ms())
		return true;
	return false;
}
...
void CTelekineticObject::keep()
{
	...
	// if (time_keep_updated + KEEP_IMPULSE_UPDATE > ::IDevice->TimeGlobal_ms()) return;
	...
	time_keep_updated = ::IDevice->TimeGlobal_ms();
...
void CTelekineticObject::fire_t(const Fvector& target, float time)
{
	...
	// time_fire_started	= ::IDevice->TimeGlobal_ms();
...
{
	if (time_raise_started + RAISE_MAX_TIME < ::IDevice->TimeGlobal_ms())
		return true;

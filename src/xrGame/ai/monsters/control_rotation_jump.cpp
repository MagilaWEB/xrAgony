...
void CControlRotationJump::on_release()
{
	...
	m_time_next_rotation_jump = ::IDevice->TimeGlobal_ms() + Random.randI(ROTATION_JUMP_DELAY_MIN, ROTATION_JUMP_DELAY_MAX);
}
...
bool CControlRotationJump::check_start_conditions()
{
	...
	if (!m_object->EnemyMan.get_enemy())
		return false;
	if (m_time_next_rotation_jump > ::IDevice->TimeGlobal_ms())
		return false;

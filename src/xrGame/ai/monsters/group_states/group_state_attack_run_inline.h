...
void CStateGroupAttackRunAbstract::initialize()
{
	...
	// interception
	m_intercept_tick = ::IDevice->TimeGlobal_ms();
	...
	// prediction
	m_memorized_tick = ::IDevice->TimeGlobal_ms();
	m_memorized_pos = this->object->EnemyMan.get_enemy()->Position();
	m_predicted_vel = cr_fvector3(0.f);

	// encirclement
	if (::IDevice->TimeGlobal_ms() > m_next_encircle_tick)
	{
		m_encircle_time = 2000 + rand() % 4000;
		m_next_encircle_tick = ::IDevice->TimeGlobal_ms() + 8000 + rand() % 8000;
	}
	else
	{
		m_encircle_time = 0;
...
{
	if (::IDevice->TimeGlobal_ms() > m_intercept_tick + m_intercept_length)
	{
		m_intercept_tick = ::IDevice->TimeGlobal_ms();
	...
	if (::IDevice->TimeGlobal_ms() > m_memorized_tick + memory_update_ms)
	{
		m_predicted_vel = (enemy_pos - m_memorized_pos) * (1000.f / (::IDevice->TimeGlobal_ms() - m_memorized_tick));
	...
	const SVelocityParam velocity_run = this->object->move().get_velocity(MonsterMovement::eVelocityParameterRunNormal);
	...
	if (!enemy_at_home || ::IDevice->TimeGlobal_ms() < this->time_state_started + m_encircle_time)
	{
...
//
// 	//	if ( g_bDogsDirMode )
// 	if ( ::IDevice->TimeGlobal_ms()-time_state_started < m_max_encircle_time )
// 	{

...
void anti_aim_ability::activate()
{
	...
	m_last_activated_tick = ::IDevice->TimeGlobal_ms();

	m_animation_hit_tick =
		::IDevice->TimeGlobal_ms() + (TTime)(m_object->anim().get_animation_hit_time(eAnimAntiAimAbility, 0) * 1000);
	...
	m_animation_end_tick = ::IDevice->TimeGlobal_ms() + (TTime)(anim_length * 1000);
...
void anti_aim_ability::start_camera_effector()
{
	...
	m_camera_effector_end_tick = ::IDevice->TimeGlobal_ms() + (TTime)(cam_eff->GetAnimatorLength() * 1000);
...
void anti_aim_ability::deactivate()
{
	...
	m_effector_id = 0;
	m_last_detection_tick = ::IDevice->TimeGlobal_ms();
	m_last_angle = M_PI;
...
void anti_aim_ability::update_schedule()
{
	...
	if (is_active())
	{
		...
		if (::IDevice->TimeGlobal_ms() < m_animation_hit_tick)
		{
			return;
		}
		if (m_effector_id == 0)
		...
		if (::IDevice->TimeGlobal_ms() < m_camera_effector_end_tick)
		{
			return;
		}
		do_deactivate();
	...
	if (::IDevice->TimeGlobal_ms() < m_last_activated_tick + (TTime)(m_timeout * 1000))
	{
#ifdef DEBUG_STATE
	...
	float const detect_delta = (::IDevice->TimeGlobal_ms() - m_last_detection_tick) / 1000.f;
	m_last_detection_tick = ::IDevice->TimeGlobal_ms();

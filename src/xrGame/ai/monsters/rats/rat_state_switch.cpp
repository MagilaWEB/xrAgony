...
const CEntityAlive* CAI_Rat::get_enemy() { return memory().enemy().selected(); }
...
void CAI_Rat::set_previous_query_time() { m_previous_query_time = ::IDevice->TimeGlobal_ms(); }
...
CAI_Rat::ERatStates CAI_Rat::get_state()
{
	return ERatStates(dwfChooseAction(m_dwActionRefreshRate, m_fAttackSuccessProbability, m_fAttackSuccessProbability,
		m_fAttackSuccessProbability, m_fAttackSuccessProbability, g_Team(), g_Squad(), g_Group(), aiRatAttackMelee,
		aiRatAttackMelee, aiRatAttackMelee, aiRatRetreat, aiRatRetreat, this, 30.f));
}
...
{
	if ((::IDevice->TimeGlobal_ms() - m_previous_query_time > TIME_TO_GO) || !m_previous_query_time)
	{
...
void CAI_Rat::set_way_point() { m_tGoalDir = get_next_target_point(); }
bool CAI_Rat::switch_if_no_enemy()
{
	if (!switch_if_enemy() ||
		(switch_if_enemy() &&
			(!switch_if_alife() ||
				((::IDevice->TimeGlobal_ms() - memory().memory(memory().enemy().selected()).m_level_time > m_dwRetreatTime) &&
...
{
	if (switch_if_enemy() &&
		(::IDevice->TimeGlobal_ms() - memory().memory(memory().enemy().selected()).m_level_time >= m_dwLostMemoryTime))
	{
		memory().enable(memory().enemy().selected(), false);
		return true;
	}
...
{
	if (::IDevice->TimeGlobal_ms() - memory().memory(memory().enemy().selected()).m_level_time >= m_dwLostRecoilTime)
	{
		return true;
	}
	return false;
...
void CAI_Rat::set_goal_time(float f_val) { m_fGoalChangeTime = 0; }
...
{
	if (time_to_next_attack + time_old_attack < ::IDevice->TimeGlobal_ms())
		return true;
	return false;
}

...
{
	unload();
	xr_delete(::ScriptEngine); // XXX: wrapped into try..catch(...) in vanilla source
...
void CAI_Space::RegisterScriptClasses()
{
	...
	for (u32 i = 0; i < registratorCount; i++)
	{
		...
		if (!::ScriptEngine->functor(I, result))
		{
			::ScriptEngine->script_log(LuaMessageType::Error, "Cannot load class registrator %s!", I);
			continue;
		}
...
void CAI_Space::LoadCommonScripts()
{
	...
	if (l_tpIniFile->line_exist("common", "script"))
	{
		...
		for (u32 i = 0; i < scriptCount; i++)
		{
			...
			::ScriptEngine->load_file(scriptName, LUA_GLOBAL);
		}
	}
	xr_delete(l_tpIniFile);
#endif
...
void CAI_Space::SetupScriptEngine()
{
	...
	::ScriptEngine->init(XRay::ScriptExporter::Export, true);
	...
	::ScriptEngine->SetUserdataObjectLoggerFunc(DumpLuaBindUserdata);
}

	...
	const GameGraph::_GRAPH_ID& game_vertex_id, const u32& level_vertex_id, const Fvector& position)
{
	...
	//	Msg
	//("[%6d][%s][%f][%f][%f]",::IDevice->TimeGlobal_ms(),object().name_replace(),VPUSH(m_destination.m_position));
}
...
{
	//	Msg
	//("[%6d][%s][%s]",::IDevice->TimeGlobal_ms(),object().name_replace(),*task->patrol_path_name());
	target(*task);
}
...
void CALifeMonsterDetailPathManager::update()
{
	...
	//		Msg							("[detail::update][%6d][%s]",::IDevice->TimeGlobal_ms(),object().name_replace());
...
void CALifeMonsterDetailPathManager::follow_path(const ALife::_TIME_ID& time_delta)
{
	...
	for (; m_path.size() > 1;)
	{
		...
		//%d",::IDevice->TimeGlobal_ms(),object().name_replace(),object().m_tGraphID,(GameGraph::_GRAPH_ID)m_path.back());

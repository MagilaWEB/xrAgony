...
IC const Fvector& moving_object::position() const { return (m_position); }
IC float moving_object::radius() const { return (m_object->Radius()); }
...
{
	m_action_frame = ::IDevice->getFrame();
	...
	m_action_time = ::IDevice->TimeGlobal_ms();
#if 0 // def DEBUG
	Msg					("%6d %s %s",::IDevice->getFrame(),*object().cName(),action == moving_object::action_wait ? "wait" : "move");
#endif // DEBUG
}
...
IC const moving_object::action_type& moving_object::action() const { return (m_action); }
IC const Fvector& moving_object::action_position() const { return (m_action_position); }
IC const u32& moving_object::action_frame() const { return (m_action_frame); }

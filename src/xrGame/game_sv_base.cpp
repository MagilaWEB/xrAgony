...
{
	// loading scripts
	auto& scriptEngine = *::ScriptEngine;
...
{
	P.w_begin(M_EVENT);
	P.w_u32(Level().timeServer()); // ::IDevice->TimerAsync_ms());
	P.w_u16(type);
	P.w_u16(dest);
}
...
void game_sv_GameState::Update()
{
	if (Level().game)
	{
		CScriptProcess* script_process = ::ScriptEngine->script_process(ScriptProcessor::Game);
		if (script_process)

...
BOOL CAnimatorCamEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	m_objectAnimator->Update(::IDevice->TimeDelta_sec());
	...
	if (!m_bAbsolutePositioning)
	{
		...
		Mdef.c = info.p;
		//		Msg("fr[%d] %2.3f,%2.3f,%2.3f", ::IDevice->getFrame(),m.c.x,m.c.y,m.c.z);
		Fmatrix mr;
		mr.mul(Mdef, m);
		info.d = mr.k;
		info.n = mr.j;
...
{
	float f = (m_end_time - IDevice->TimeGlobal_sec()) / m_life_time;
...
void SndShockEffector::Start(CActor* A, float snd_length, float power)
{
	...
	m_end_time = IDevice->TimeGlobal_sec() + m_life_time;
...
{
	m_cur_length += IDevice->TimeDelta_ms();
...
BOOL CControllerPsyHitCamEffector::ProcessCam(SCamEffectorInfo& info)
{
	...
	if (angle_lerp(m_dangle_current.x, m_dangle_target.x, ANGLE_SPEED, ::IDevice->TimeDelta_sec()))
	{
		m_dangle_target.x = angle_normalize(Random.randFs(DELTA_ANGLE_X));
	}

	if (angle_lerp(m_dangle_current.y, m_dangle_target.y, ANGLE_SPEED, ::IDevice->TimeDelta_sec()))
	{
		m_dangle_target.y = angle_normalize(Random.randFs(DELTA_ANGLE_Y));
	}

	if (angle_lerp(m_dangle_current.z, m_dangle_target.z, ANGLE_SPEED, ::IDevice->TimeDelta_sec()))
	{
	...
	m_time_current += ::IDevice->TimeDelta_sec();

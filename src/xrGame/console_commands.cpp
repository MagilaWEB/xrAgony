...
class CCC_СhangeGameTime : public CCC_Vector3
{
private:
	Fvector p;
public:
	CCC_СhangeGameTime(LPCSTR N) : CCC_Vector3(N, &p, Fvector(), Fvector().set(FLT_MAX, FLT_MAX, FLT_MAX)) {}
	virtual void Execute(LPCSTR args)
	{
		if (!xr_strlen(args))
			return;
...
void CCC_RegisterCommands()
{
	...
	if (Core.ParamFlags.test(Core.dev))
	{
		...
		CMD1(CCC_JumpToLevel, "jump_to_level");
		CMD3(CCC_Mask, "g_god", &psActorFlags, AF_GODMODE);
		CMD1(CCC_СhangeGameTime, "g_change_game_time");
		CMD3(CCC_Mask, "g_unlimitedammo", &psActorFlags, AF_UNLIMITEDAMMO);
		CMD1(CCC_TimeFactor, "time_factor");
		CMD1(CCC_Script, "run_script");
		CMD1(CCC_ScriptCommand, "run_string");

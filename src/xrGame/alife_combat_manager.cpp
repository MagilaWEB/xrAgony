...
smart_cast<CSE_ALifeSmartZone*>(tpALifeSchedulable1);
			return										(!smart_zone ? false : randF(100) <
100.f*smart_zone->detect_probability());
		}
		default :										NODEFAULT;
	}
#ifdef DEBUG
...
m_tpaCombatGroups[iCombatGroupIndex][0]->m_tpBestDetector;
	...
	return										(randF(100) < ((eCombatTypeMonsterMonster != combat_type()) ?
...
smart_cast<CSE_ALifeMonsterAbstract*>(m_tpaCombatGroups[iCombatGroupIndex ^ 1][l_iIndex]);
				R_ASSERT2					(l_tpALifeMonsterAbstract,"Invalid combat object");
...
#ifdef DEBUG
				if (psAI_Flags.test(aiALife)) {
					Msg						("[LSS] %s %s %s [power %5.2f][damage %5.2f][health %5.2f][creatures left
	...
	for (int i=0; i<2; ++i) {
		...
		if (l_tpALifeGroupAbstract) {
			for (int I=0, N=l_tpALifeGroupAbstract->m_tpMembers.size() ; I<N; ++I) {
				CSE_ALifeMonsterAbstract	*l_tpALifeMonsterAbstract =
smart_cast<CSE_ALifeMonsterAbstract*>(objects().object(l_tpALifeGroupAbstract->m_tpMembers[I]));

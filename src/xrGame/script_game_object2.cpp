...
void CScriptGameObject::explode(u32 level_time)
{
	...
	if (object().H_Parent())
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CExplosive : cannot explode object wiht parent!");
		return;
	}

	if (!explosive)
		::ScriptEngine->script_log(LuaMessageType::Error, "CExplosive : cannot access class member explode!");
	else
	{
		Fvector normal;
...
void CScriptGameObject::set_item(MonsterSpace::EObjectAction object_action)
{
	CObjectHandler* object_handler = smart_cast<CAI_Stalker*>(&object());
	if (!object_handler)
		::ScriptEngine->script_log(LuaMessageType::Error, "CObjectHandler : cannot access class member set_item!");
	else
...
void CScriptGameObject::play_cycle(LPCSTR anim, bool mix_in)
{
	...
	if (sa)
	{
		...
		{
			::ScriptEngine->script_log(LuaMessageType::Error, "CGameObject : has not cycle %s", anim);
		}
	}
	else
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CGameObject : is not animated object");
	}
}
...
u32 CScriptGameObject::memory_time(const CScriptGameObject& lua_game_object)
{
	...
	if (!monster)
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CScriptEntity : cannot access class member memory!");
		return (0);
	}
	else
...
CCar* CScriptGameObject::get_car()
{
	...
	if (!car)
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CGameObject : cannot access class member get_car!");
		NODEFAULT;
	}
	return car;
}

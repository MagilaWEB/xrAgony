...
IC const xr_vector<CSoundPlayer::CSoundSingle>& CSoundPlayer::playing_sounds() const { return (m_playing_sounds); }
IC u32 CSoundPlayer::active_sound_count(bool only_playing) const
{
	...
	{
		for (; I != E; ++I)
			if ((*I).m_sound->_feedback() || ((*I).m_start_time <= ::IDevice->TimeGlobal_ms()))
				++count;
	}
	else
	{

...
IC float CMovementManager::speed() const { return (m_speed); }
IC bool CMovementManager::path_completed() const { return ((m_path_state == ePathStatePathCompleted) && actual()); }
IC const float& CMovementManager::old_desirable_speed() const { return (m_old_desirable_speed); }
IC void CMovementManager::set_desirable_speed(float speed) { m_old_desirable_speed = speed; }
IC void CMovementManager::set_body_orientation(const CBoneRotation& orientation) { m_body = orientation; }
IC const CMovementManager::CBoneRotation& CMovementManager::body_orientation() const { return (m_body); }
template <typename T>
...
IC void CMovementManager::set_build_path_at_once() { m_build_at_once = true; }
IC CMovementManager::CGameVertexParams* CMovementManager::base_game_params() const { return (m_base_game_selector); }

...
void CDelayedActionFuse::SetTimer(float current_condition)
{
	...
	m_fTime += IDevice->TimeGlobal_sec(); //+current_condition/m_fSpeedChangeCondition;
...
float CDelayedActionFuse::Time()
{
	...
	if (!isActive())
		...
		return m_fTime - IDevice->TimeGlobal_sec();
}
...
bool CDelayedActionFuse::Update(float current_condition)
{
	...
	bool ret = false;
	float l_time_to_explosion = m_fTime - IDevice->TimeGlobal_sec();

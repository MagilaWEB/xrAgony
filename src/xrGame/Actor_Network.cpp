...
void CActor::net_Export(NET_Packet& P) // export to server
{
	...
	P.w_float /*w_angle8*/(angle_normalize(r_model_yaw)); // ::IDevice->cast()->vCameraDirection.getH());//
	...
	/*
		if (OnClient())
		{
			if (!pStatGraph)
			{
				...
				pStatGraph->SetRect(0, g_Y, ::IDevice->cast()->dwWidth, 100, 0xff000000, 0xff000000);
				g_Y += 110;
				if (g_Y > 700) g_Y = 100;
...
void CActor::SetCallbacks()
{
	...
	V->LL_GetBoneInstance(u16(m_spine1)).set_callback(bctCustom, Spin0Callback, this);
	V->LL_GetBoneInstance(u16(m_spine2)).set_callback(bctCustom, Spin1Callback, this);

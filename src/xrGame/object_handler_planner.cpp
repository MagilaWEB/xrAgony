	...
	u32 min_queue_size, u32 max_queue_size, u32 min_queue_interval, u32 max_queue_interval)
{
	...
	if (m_use_log)
	{
		Msg("%6d : Active item %s", ::IDevice->TimeGlobal_ms(), object().inventory().ActiveItem() ?
				*object().inventory().ActiveItem()->object().cName() :
				"no active items");
		Msg("%6d : Goal %s", ::IDevice->TimeGlobal_ms(), property2string(condition_id));
	}
#endif
	CState condition;
	...
	if ((m_min_queue_size != min_queue_size) || (m_max_queue_size != max_queue_size) ||
		...
		(m_next_time_change <= ::IDevice->TimeGlobal_ms()))
	{
		...
		m_next_time_change = ::IDevice->TimeGlobal_ms() + m_queue_interval;

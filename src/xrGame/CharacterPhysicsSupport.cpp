...
CCharacterPhysicsSupport::CCharacterPhysicsSupport(EType atype, CEntityAlive* aentity)
	: m_pPhysicsShell(aentity->PPhysicsShell()), m_EntityAlife(*aentity), mXFORM(aentity->XFORM()),
	  m_ph_sound_player(aentity), m_interactive_motion(0), m_PhysicMovementControl(new CPHMovementControl(aentity)),
	  m_eType(atype), m_eState(esAlive), m_physics_skeleton(nullptr), m_ik_controller(nullptr), m_BonceDamageFactor(1.f),
	  ...
	  m_hit_valide_time(u32(-1)), m_collision_activating_delay(nullptr)
{
	m_flags.assign(0);
...
{
	m_sv_hit = H;
	m_hit_valide_time = ::IDevice->TimeGlobal_ms() + hit_valide_time;
	...
	if (m_flags.test(fl_block_hit))
	{
		...
		if (::IDevice->TimeGlobal_ms() - m_EntityAlife.GetLevelDeathTime() >= 2000)
...
{
	m_physics_shell_animated_time_destroy = ::IDevice->TimeGlobal_ms() + physics_shell_animated_destroy_delay;
...
void CCharacterPhysicsSupport::update_animation_collision()
{
	if (animation_collision())
	{
		...
		if (::IDevice->TimeGlobal_ms() > m_physics_shell_animated_time_destroy)
			destroy_animation_collision();
	}
}
#ifdef DEBUG
	...
	IGameObject* who, const Fvector& inital_entity_position, const Fvector& dp, const Fvector& velocity)
{
	...
	// anim_mov_blend->timeCurrent + ::IDevice->TimeDelta_sec()*anim_mov_blend->speed <
	...
	//	anim_mov_blend->timeCurrent  += ::IDevice->TimeDelta_sec() * anim_mov_blend->speed;
	//	m_pPhysicsShell->AnimToVelocityState( ::IDevice->TimeDelta_sec(), 2 * default_l_limit, 10.f * default_w_limit );
	//	mXFORM.set( sv_xform );
	//}
	IKinematics* K = smart_cast<IKinematics*>(m_EntityAlife.Visual());
...
{
	if (m_hit_valide_time < ::IDevice->TimeGlobal_ms() || !m_sv_hit.is_valide())
	{
		if (m_EntityAlife.use_simplified_visual())
			return;
		ActivateShell(nullptr);

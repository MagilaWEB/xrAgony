	...
	float time_delta, float object_velocity, float luminocity) const
{
	...
	if (::ScriptEngine->functor("visual_memory_manager.get_visible_value", funct))
		return (funct(m_object ? m_object->lua_game_object() : 0, game_object ? game_object->lua_game_object() : 0,
			time_delta, current_state().m_time_quant, luminocity, current_state().m_velocity_factor, object_velocity,
			distance, object_distance, always_visible_distance));
	//-Alundaio
#endif
	...
	if (distance < object_distance)
	{
		if (object)
		{
			...
			if (object->m_value < 0.f)
				...
				object->m_update_time = ::IDevice->TimeGlobal_ms();
...
void CVisualMemoryManager::add_visible_object(const IGameObject* object, float time_delta, bool fictitious)
{
	...
	if (m_objects->end() == J)
	{
		...
		visible_object.m_first_level_time = ::IDevice->TimeGlobal_ms();
#endif
...
void CVisualMemoryManager::update(float time_delta)
{
	...
	START_PROFILE("Memory Manager/visuals/update/make_invisible")
	{
		...
		xr_vector<CVisibleObject>::iterator E = m_objects->end();
		for (; I != E; ++I)
			if ((*I).m_level_time + current_state().m_still_visible_time < ::IDevice->TimeGlobal_ms())
	...
	START_PROFILE("Memory Manager/visuals/update/make_not_yet_visible")
	{
		for (CNotYetVisibleObject & not_yet_visible_object : m_not_yet_visible_objects)
			if (not_yet_visible_object.m_update_time < ::IDevice->TimeGlobal_ms())

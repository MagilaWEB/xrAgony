...
void CStepManager::on_animation_start(MotionID motion_id, CBlend* blend)
{
	...
	m_time_anim_started = ::IDevice->TimeGlobal_ms();
...
void CStepManager::update(bool b_hud_view)
{
	...
	float dist_sqr = m_object->Position().distance_to_sqr(::IDevice->cast()->vCameraPosition);
	...
	for (u32 i = 0; i < m_legs_count; i++)
	{
		...
		if (offset_time <= cur_time)
		{
			...
			if (CGameObject* n_object = smart_cast<CGameObject*>(m_object))
				n_object->callback(GameObject::eOnFootStep)(n_object->lua_game_object(), m_step_info.params.step[i].power, b_play, is_on_ground(), b_hud_view);

...
IC void CSoundMemoryManager::update_sound_threshold()
{
	...
	m_sound_threshold =
		_max(m_self_sound_factor * m_sound_threshold * exp(float(::IDevice->TimeGlobal_ms() - m_last_sound_time) /
	...
	IGameObject* object, int sound_type, CSound_UserDataPtr user_data, const Fvector& position, float sound_power)
{
	...
	Msg("%s (%d) - sound type %x from %s at %d in (%.2f,%.2f,%.2f) with power %.2f", *self->cName(),
		::IDevice->TimeGlobal_ms(), sound_type, object ? *object->cName() : "world", ::IDevice->TimeGlobal_ms(), position.x,
	...
	m_last_sound_time = ::IDevice->TimeGlobal_ms();
	VERIFY(_valid(m_sound_threshold));
...
void CSoundMemoryManager::add(const IGameObject* object, int sound_type, const Fvector& position, float sound_power)
{
	...
	(
		object &&
		object->H_Parent() &&
		smart_cast<const CEntityAlive*>(object->H_Parent()) &&
		(
			me_entity_alive->tfGetRelationType(smart_cast<const CEntityAlive*>(object->H_Parent())) == ALife::eRelationTypeFriend ||
			me_entity_alive->tfGetRelationType(smart_cast<const CEntityAlive*>(object->H_Parent())) == ALife::eRelationTypeNeutral
		)
	)
		return;
	...
	(
		...
		me_entity_alive &&
		(
			me_entity_alive->tfGetRelationType(who_entity_alive) == ALife::eRelationTypeFriend ||
			me_entity_alive->tfGetRelationType(who_entity_alive) == ALife::eRelationTypeNeutral
		)
	)
		return;
	...
	if (heavy_sound && sound_power >= 4.f)
	{
		if (CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(m_object))
		{
			if (who_entity_alive && who_entity_alive->g_Alive() && !m_object->memory().visual().visible_now(who_entity_alive))
			{
				...
				m_object->memory().visual().add_visible_object(object, ::IDevice->TimeDelta_sec());
	...
	if (m_sounds->end() == J)
	{
		...
		sound_object.m_first_level_time = ::IDevice->TimeGlobal_ms();
#endif
		add(sound_object);
	}
	else
...
void CSoundMemoryManager::update()
{
	...
	std::erase_if(*m_sounds, [](const CSoundObject& object) {
		if (!object.m_object)
			return (false);
...
{
	SOUNDS::iterator I = m_sounds->find_if([sound_object](const MemorySpace::CSoundObject& object)
	{
...
#ifdef USE_LEVEL_TIME
		packet.w_u32((::IDevice->TimeGlobal_ms() > (*I).m_level_time) ? (::IDevice->TimeGlobal_ms() - (*I).m_last_level_time) : 0);
#endif // USE_LAST_LEVEL_TIME
#ifdef USE_FIRST_LEVEL_TIME

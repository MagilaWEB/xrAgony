...
void CScriptEntity::SetScriptControl(const bool bScriptControl, shared_str caSciptName)
{
	...
	#ifdef DEBUG
		if (bScriptControl)
			::ScriptEngine->script_log			(ScriptStorage::eLuaMessageTypeInfo,"Script %s set object %s
	under its control",*caSciptName,*object().cName());
		else
			::ScriptEngine->script_log			(ScriptStorage::eLuaMessageTypeInfo,"Script %s freed object %s
...
bool CScriptEntity::bfAssignMovement(CScriptEntityAction* tpEntityAction)
{
	...
	if (!m_monster)
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "Cannot assign a movement action not to a monster!");
		return (true);
	}

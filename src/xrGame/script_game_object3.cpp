	...
	float min_enemy_distance, float max_enemy_distance)
{
	...
	if (!stalker)
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CGameObject : cannot access class member best_cover!");
		return (0);
	}
...
CScriptGameObject* CScriptGameObject::GetFood() const
{
	...
	if (!l_tpStalker)
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CAI_Stalker : cannot access class member GetFood!");
		return (0);
	}
	...
	LPCSTR path_name, const EPatrolStartType patrol_start_type, const EPatrolRouteType patrol_route_type, bool random)
{
	CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
		::ScriptEngine->script_log(LuaMessageType::Error, "CAI_Stalker : cannot access class member movement!");
	else
...
u32 CScriptGameObject::get_dest_game_vertex_id()
{
	CAI_Stalker *stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
		::ScriptEngine->script_log(LuaMessageType::Error, "CAI_Stalker : cannot access class member get_dest_level_vertex_id!");
	else
...
CHARACTER_RANK_VALUE CScriptGameObject::GetRank()
{
	...
	if (!stalker)
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CAI_Stalker : cannot access class member GetRank!");
		...
		if (mental_state != eMentalStateDanger) {
			if (stalker->brain().initialized()) {
				if (stalker->brain().current_action_id() == StalkerDecisionSpace::eWorldOperatorCombatPlanner) {
					::ScriptEngine->script_log	(LuaMessageType::Error,"CAI_Stalker : set_mental_state is used during universal combat!, object[%s]", stalker->cName().c_str());
//					return;
				}
			}
		}
...
MonsterSpace::EBodyState CScriptGameObject::body_state() const
{
	...
	if (!stalker)
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CAI_Stalker : cannot access class member body_state!");
...
MovementManager::EPathType CScriptGameObject::path_type() const
{
	...
	if (!stalker)
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "CAI_Stalker : cannot access class member path_type!");
...
void CScriptGameObject::set_sight(SightManager::ESightType sight_type, Fvector* vector3d, u32 dwLookOverDelay)
{
	CAI_Stalker* stalker = smart_cast<CAI_Stalker*>(&object());
	if (!stalker)
		::ScriptEngine->script_log(LuaMessageType::Error, "CSightManager : cannot access class member set_sight!");
	else
	{
...
bool CScriptGameObject::weapon_strapped() const
{
	...
	//	Msg					( "[%6d][%s] weapon_strapped = %s", ::IDevice->TimeGlobal_ms(), stalker->cName().c_str(), result ?
	//"true"
	//: "false" );
	return (result);
}
...
bool CScriptGameObject::weapon_unstrapped() const
{
	...
	//	Msg					( "[%6d][%s] weapon_unstrapped = %s", ::IDevice->TimeGlobal_ms(), stalker->cName().c_str(), result
	//?
	//"true" : "false" );
	return (result);
}
...
void CScriptGameObject::AttachVehicle(CScriptGameObject* veh, const bool bForce)
{
	...
	if (actor)
	{
		...
		if (vehicle)
			...
			::ScriptEngine->script_log(LuaMessageType::Error, "CGameObject : cannot be cast to CHolderCustom!");
	}
}

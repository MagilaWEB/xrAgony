...
void CActor::g_fireParams(const CHudItem* pHudItem, Fvector& fire_pos, Fvector& fire_dir)
{
	...
	if (weapon && !pMissile)
	{
		if (smart_cast<CWeaponKnife*>(weapon))
		{
			SetDirectCams();
			if (cam_active != eacFirstEye)
				fire_pos = weapon->Position();
		}
		else
		{
	...
	if (pMissile && !weapon)
	{
		...
		{
			Fvector result;
			Visual()->dcast_PKinematics()->LL_GetTransform(u16(m_r_finger2)).transform_tiny(result, Fvector().set(.0f, .0f, .0f));
	...
	for (int i = 0; i < 4; i++)
	{
		if (inventory().ItemFromSlot(BestWeaponSlots[i]))
		{
			if (inventory().GetActiveSlot() != BestWeaponSlots[i])
			{
				...
				{
#ifdef DEBUG
					Msg("--- Selecting best weapon [%d], Frame[%d]", BestWeaponSlots[i], ::IDevice->getFrame());
#endif // #ifdef DEBUG

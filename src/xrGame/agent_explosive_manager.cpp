...
void CAgentExplosiveManager::register_explosive(const CExplosive* explosive, const CGameObject* game_object)
{
	...
	m_explosives.push_back(CDangerExplosive(explosive, game_object, 0, ::IDevice->TimeGlobal_ms()));
	...
	object().location().add(new CDangerObjectLocation(game_object, ::IDevice->TimeGlobal_ms(), interval, GRENADE_RADIUS));
}

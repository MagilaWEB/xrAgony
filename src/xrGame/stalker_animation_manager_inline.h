...
IC CStalkerAnimationPair& CStalkerAnimationManager::global() { return (m_global); }
IC CStalkerAnimationPair& CStalkerAnimationManager::head() { return (m_head); }
IC CStalkerAnimationPair& CStalkerAnimationManager::torso() { return (m_torso); }
IC CStalkerAnimationPair& CStalkerAnimationManager::legs() { return (m_legs); }
IC CStalkerAnimationPair& CStalkerAnimationManager::script() { return (m_script); }
...
{
	return (m_global_selector || m_global_callback || m_global.need_update() || m_head.need_update() ||
		m_torso.need_update() || m_legs.need_update());
}

IC const float& CStalkerAnimationManager::target_speed() const { return (m_last_non_zero_speed); }
IC void CStalkerAnimationManager::special_danger_move(const bool& value) { m_special_danger_move = value; }
IC const bool& CStalkerAnimationManager::special_danger_move() const { return (m_special_danger_move); }
...
IC void CStalkerAnimationManager::global_selector(AnimationSelector const& selector) { m_global_selector = selector; }
...
IC void CStalkerAnimationManager::global_callback(AnimationCallback const& callback) { m_global_callback = callback; }
...
IC void CStalkerAnimationManager::global_modifier(AnimationModifier const& modifier) { m_global_modifier = modifier; }

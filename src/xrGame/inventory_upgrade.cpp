...
void Upgrade::construct(const shared_str& upgrade_id, Group& parental_group, Manager& manager_r)
{
    ...
    R_ASSERT2(::ScriptEngine->functor(precondition_functor_str, m_preconditions.functr),
        make_string(
    ...
    R_ASSERT2(::ScriptEngine->functor(effect_functor_str, m_effects.functr),
    ...
    R_ASSERT2(::ScriptEngine->functor(prereq_functor_str, m_prerequisites.functr),
        make_string("Failed to get prerequisites functor in section[%s], functor[%s]", id_str(), prereq_functor_str));
    m_prerequisites();

    /*R_ASSERT2(
        ::ScriptEngine->functor( tooltip_functor_str, m_tooltip.functr ),

...
void CHangingLamp::UpdateCL()
{
	...
	{
		if (Visual() && ::IDevice->cast()->ViewFromMatrix.testSphere_dirty(spatial.sphere.P, spatial.sphere.R + light_render->get_homdata().sphere.R))
		...
		{
			int frame;
			u32 clr = lanim->CalculateBGR(IDevice->TimeGlobal_sec(), frame); // возвращает в формате BGR
			Fcolor fclr;

...
{
	dbg_ph_draw_mode = dmSecondaryThread;
	cash_draw_remove_time = remove_time + ::IDevice->TimeGlobal_ms();
}
...
void DBG_PHAbstructRender()
{
	...
	if (dbg_ph_draw_mode != dmCashed)
	{
		...
		if (cash_draw_remove_time < ::IDevice->TimeGlobal_ms())
		{
...
{
	if (Device.Paused() || ::IDevice->getFrame() == previous_frame || !(::IDevice->TimeDelta_sec() > EPS_S))
		return;
	draw_frame = !draw_frame;
	clear_vector(dbg_draw_simple);
	previous_frame = ::IDevice->getFrame();

...
void CALifeStorageManager::save(LPCSTR save_name_no_check, bool update_name)
{
	...
	if (::ScriptEngine->functor("alife_storage_manager.CALifeStorageManager_before_save", funct1))
...
#ifdef ENGINE_LUA_ALIFE_STORAGE_MANAGER_CALLBACKS
	::ScriptEngine->function_event<void>("alife_storage_manager.CALifeStorageManager_save", static_cast<pcstr>(m_save_name));
#endif
	//-Alundaio
...
#ifdef ENGINE_LUA_ALIFE_STORAGE_MANAGER_CALLBACKS
	::ScriptEngine->function_event<void>("alife_storage_manager.CALifeStorageManager_load", file_name);
#endif
	//-Alundaio

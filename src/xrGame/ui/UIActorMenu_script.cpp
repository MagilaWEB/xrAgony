...
void CUIActorMenu::TryRepairItem(CUIWindow* w, void* d)
{
	...
	R_ASSERT2(::ScriptEngine->functor("inventory_upgrades.can_repair_item", funct),
	...
	R_ASSERT2(::ScriptEngine->functor("inventory_upgrades.question_repair_item", funct2),
...
void CUIActorMenu::RepairEffect_CurItem()
{
	...
	R_ASSERT(::ScriptEngine->functor("inventory_upgrades.effect_repair_item", funct));
...
bool CUIActorMenu::CanUpgradeItem(PIItem item)
{
	...
	luabind::functor<bool> funct;
	R_ASSERT2(::ScriptEngine->functor("inventory_upgrades.can_upgrade_item", funct),
...
void CUIActorMenu::CurModeToScript()
{
	...
	luabind::functor<void> funct;
	R_ASSERT(::ScriptEngine->functor("actor_menu.actor_menu_mode", funct));
	funct(mode);
}

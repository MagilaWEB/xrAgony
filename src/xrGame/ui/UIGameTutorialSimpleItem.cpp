...
bool CUISequenceSimpleItem::IsPlaying()
{
	...
	return (m_time_start + m_time_length) > (::IDevice->TimeContinual() / 1000.0f);
}
...
void CUISequenceSimpleItem::OnRender()
{
	if (m_time_start < -2.0f)
		...
		m_time_start = float(::IDevice->TimeContinual()) / 1000.0f;
}
...
float CUISequenceSimpleItem::current_factor()
{
	if (m_time_start < 0.0f || fis_zero(m_time_length))
		...
		return ((::IDevice->TimeContinual() / 1000.0f) - m_time_start) / m_time_length;
}

void CUISequenceSimpleItem::Update()
{
	inherited::Update();
	float _start = (m_time_start < 0.0f) ? (float(::IDevice->TimeContinual()) / 1000.0f) : m_time_start;

	float gt = float(::IDevice->TimeContinual()) / 1000.0f;
	auto _I = m_subitems.begin();
	auto _E = m_subitems.end();
	for (; _I != _E; ++_I)
	{
...
void CUISequenceSimpleItem::OnKeyboardPress(int dik)
{
	...
	for (u32 idx = 0; idx < m_actions.size(); ++idx)
	{
		...
		if (b)
		{
			...
			bool functor_exists = ::ScriptEngine->functor(itm.m_functor.c_str(), functor_to_call);

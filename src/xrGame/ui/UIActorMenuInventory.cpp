...
void CUIActorMenu::OnInventoryAction(PIItem pItem, u16 action_type)
{
	CUIDragDropListEx* all_lists[] = {m_pInventoryBeltList, m_pInventoryBagList,
		m_pTradeActorBagList, m_pTradeActorList, nullptr};
	...
	for (u8 i = 1; i <= 4; ++i)
	{
		...
		if (functor_name)
		{
			...
			if (::ScriptEngine->functor(functor_name, funct))
			{
				...
				if (act_str)
				{
					m_UIPropertiesBox->AddItem(act_str, nullptr, INVENTORY_EAT2_ACTION + i - 1);
					b_show = true;
				}
			}
		}
...
void CUIActorMenu::ProcessPropertiesBoxClicked(CUIWindow* w, void* d)
{
	...
	{
		PIItem item = CurrentIItem(); // temporary storing because of AttachAddon is setting curiitem to nullptr

...
void CallFunction(shared_str const& func)
{
	...
	bool functor_exists = ::ScriptEngine->functor(func.c_str(), functor_to_call);
...
void CUISequenceItem::Start()
{
	...
	{
		bool functor_exists = ::ScriptEngine->functor(m_onframe_lua_function.c_str(), m_onframe_functor);
		THROW3(
...
CUISequenceItem* CUISequencer::GetNextItem()
{
	...
	while (m_sequencer_items.size())
	{
		...
		bool functor_exists = ::ScriptEngine->functor(f.c_str(), functor_to_call);

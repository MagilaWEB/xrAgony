...
void UpdateCameraDirection(CGameObject* pTo)
{
	...
	if (angle_difference(cam->yaw, -h) > 0.2)
		cam->yaw = angle_inertion_var(cam->yaw, -h, 0.15f, 0.2f, PI_DIV_6, ::IDevice->TimeDelta_sec());

	if (angle_difference(cam->pitch, -p) > 0.2)
		cam->pitch = angle_inertion_var(cam->pitch, -p, 0.15f, 0.2f, PI_DIV_6, ::IDevice->TimeDelta_sec());
}
...
bool CUITalkWnd::TopicMode() { return nullptr == m_pCurrentDialog.get(); }
void CUITalkWnd::ToTopicMode() { m_pCurrentDialog = DIALOG_SHARED_PTR((CPhraseDialog*)nullptr); }

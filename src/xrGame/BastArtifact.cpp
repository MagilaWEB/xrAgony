	...
	bool& /**do_colide/**/, bool bo1, dContact& c, SGameMtl* /*material_1*/, SGameMtl* /*material_2*/)
{
	...
	if (!pBastArtefact)
		pBastArtefact = l_pUD2 ? smart_cast<CBastArtefact*>(l_pUD2->ph_ref_object) : nullptr;
	if (!pBastArtefact)
		return;
	...
	if (!pEntityAlive)
		pEntityAlive = l_pUD2 ? smart_cast<CEntityAlive*>(l_pUD2->ph_ref_object) : nullptr;
...
void CBastArtefact::UpdateCLChild()
{
	...
	if (m_fEnergy > 0)
		m_fEnergy -= m_fEnergyDecreasePerTime * fDeltaT();
	...
	if (getVisible() && m_pPhysicsShell)
	{
		...
		if (m_AttakingEntity)
		{
			if (m_AttakingEntity->g_Alive() && m_fEnergy > m_fStrikeImpulse)
			{
				...
				m_pPhysicsShell->applyImpulse(dir, m_fStrikeImpulse * fDeltaT() * m_pPhysicsShell->getMass());
			}
			else
			{
				m_AttakingEntity = nullptr;

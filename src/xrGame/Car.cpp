...
void CCar::UpdateEx(float fov)
{
	...
	//	Log("UpdateCL",::IDevice->getFrame());
	// XFORM().set(m_pPhysicsShell->mXFORM);
	VisualUpdate(fov);
	if (OwnerActor() && OwnerActor()->IsMyCamera())
	{
		cam_Update(::IDevice->TimeDelta_sec(), fov);
...
void CCar::CreateSkeleton(CSE_Abstract* po)
{
	...
	BONE_P_MAP::iterator i = bone_map.begin(), e = bone_map.end();
	if (i != e)
...
void CCar::Init()
{
	...
	{
		m_pPhysicsShell->SetAirResistance(default_k_l * ini->r_float("air_resistance", "linear_factor"),
...
{
	b_starting = true;
	m_dwStartTime = ::IDevice->TimeGlobal_ms();
}
...
void CCar::StartBreaking()
{
	...
	{
		b_breaks = true;
		m_break_start = IDevice->TimeGlobal_sec();
	}
}
...
void CCar::UpdateBack()
{
	...
	{
		float k = 1.f;
		float time = (IDevice->TimeGlobal_sec() - m_break_start);
...
float CCar::EnginePower()
{
	...
	if (b_starting)
	{
		...
		else if (::IDevice->TimeGlobal_ms() - m_dwStartTime > 1000)
			b_starting = false;
	}
...
IC void CCar::fill_wheel_vector(LPCSTR S, xr_vector<T>& type_wheels)
{
	...
	for (int i = 0; i < count; ++i)
	{
		...
		if (J == bone_map.end())
		{
			bone_map.emplace(bone_id, physicsBone());

			SWheel& wheel = (m_wheels_map.emplace(bone_id, SWheel(this))).first->second;

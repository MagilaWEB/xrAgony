...
BOOL CPostprocessAnimator::Process(SPPInfo& PPInfo)
{
	...
	if (m_start_time < 0.0f)
		m_start_time = IDevice->TimeGlobal_sec();
	if (m_bCyclic && ((IDevice->TimeGlobal_sec() - m_start_time) > f_length))
		m_start_time += f_length;

	Update(IDevice->TimeGlobal_sec() - m_start_time);

	VERIFY(_valid(m_factor));
	VERIFY(_valid(m_factor_speed));
	VERIFY(_valid(m_dest_factor));
	if (m_bStop)
		m_factor -= ::IDevice->TimeDelta_sec() * m_factor_speed;
	else
		m_factor += m_factor_speed * ::IDevice->TimeDelta_sec() * (m_dest_factor - m_factor);

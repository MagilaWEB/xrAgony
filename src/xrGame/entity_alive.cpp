	  ...
	  m_hit_bone_surface_areas_actual(false)
{
	...
	m_use_timeout = 5000;
	m_used_time = ::IDevice->TimeGlobal_ms();
	m_squad_index = u8(-1);
...
void CEntityAlive::shedule_Update(u32 dt)
{
	...
	if (Local() && !g_Alive() && !AlreadyDie())
	{
		if (conditions().GetWhoHitLastTime())
		{
			...
			//%s",::IDevice->TimeGlobal_ms(),*cName());
			KillEntity(conditions().GetWhoHitLastTimeID());
		}
		else
		{
			//			Msg			("%6d : KillEntity from CEntityAlive for object %s",::IDevice->TimeGlobal_ms(),*cName());
			KillEntity(ID());
		}
	}
}
...
void CEntityAlive::UpdateFireParticles()
{
	...
	auto const& wounds = conditions().wounds();
	auto it = wounds.begin();
	auto it_e = wounds.end();
...
void CEntityAlive::UpdateBloodDrops()
{
	...
	for (; it != it_e; ++it)
	{
		...
		if (pWound->m_fDropTime < IDevice->TimeGlobal_sec())
		{
			float size_k = blood_size - m_fStopBloodWoundSize;
			size_k = size_k < 1.f ? size_k : 1.f;
			pWound->m_fDropTime = IDevice->TimeGlobal_sec() +
...
bool CEntityAlive::is_locked_corpse()
{
	if (!b_eating)
	{
		if (m_used_time + m_use_timeout > ::IDevice->TimeGlobal_ms())
		{
			return true;
		}
	}
...
void CEntityAlive::fill_hit_bone_surface_areas() const
{
	...
	m_hit_bone_surface_areas.resize(0);

...
struct XRECORE_API SESubCommand
{
    SECommand* parent;
    xr_string desc;
    CCommandVar p0;
...
XRECORE_API CCommandVar ExecCommand(u32 cmd, CCommandVar p1 = u32(0), CCommandVar p2 = u32(0));
XRECORE_API CCommandVar ExecCommand(const xr_shortcut& val);
XRECORE_API
...
XRECORE_API ECommandVec& GetEditorCommands();
XRECORE_API SESubCommand* FindCommandByShortcut(const xr_shortcut& val);
XRECORE_API BOOL LoadShortcuts(CInifile* ini);
XRECORE_API BOOL SaveShortcuts(CInifile* ini);
XRECORE_API BOOL AllowLogCommands();

...
{
	VERIFY((this != &A) && (this != &B));
	__m128 row1 = _mm_loadu_ps((const float*)&A.m[0][0]);
	__m128 row2 = _mm_loadu_ps((const float*)&A.m[1][0]);
	__m128 row3 = _mm_loadu_ps((const float*)&A.m[2][0]);
	__m128 row4 = _mm_loadu_ps((const float*)&A.m[3][0]);

	for (u32 i = 0; i < 4; i++)
		_mm_storeu_ps((float*)&m[i][0], _mm_add_ps(
			_mm_add_ps(
				_mm_add_ps(
					_mm_mul_ps(row1, _mm_set1_ps(B.m[i][0])),
					_mm_mul_ps(row2, _mm_set1_ps(B.m[i][1]))),
...
_matrix<float>& _matrix<float>::mul_43(const Self& A, const Self& B)
{
	VERIFY((this != &A) && (this != &B));
	__m128 row1 = _mm_loadu_ps(&A.m[0][0]);
	__m128 row2 = _mm_loadu_ps(&A.m[1][0]);
...
_matrix<T>& _matrix<T>::mul_43(const Self& A, const Self& B)
{
	...
	_mm_storeu_ps((float*)&m[3][0], _mm_add_ps(
		_mm_add_ps(
			_mm_mul_ps(row1, _mm_set1_ps(B.m[3][0])),
			_mm_mul_ps(row2, _mm_set1_ps(B.m[3][1]))),
		_mm_add_ps(_mm_mul_ps(row3, _mm_set1_ps(B.m[3][2])), row4))

...
void CObjectFactory::register_script_class(LPCSTR client_class, LPCSTR server_class, LPCSTR clsid, LPCSTR script_clsid)
{
	...
	if (!::ScriptEngine->function_object(client_class, client, LUA_TUSERDATA))
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "Cannot register class %s", client_class);
		return;
	}
#endif
	luabind::object server;
	if (!::ScriptEngine->function_object(server_class, server, LUA_TUSERDATA))
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "Cannot register class %s", server_class);
		return;
	}
...
void CObjectFactory::register_script_class(LPCSTR unknown_class, LPCSTR clsid, LPCSTR script_clsid)
{
	...
	if (!::ScriptEngine->function_object(unknown_class, creator, LUA_TUSERDATA))
	{
		::ScriptEngine->script_log(LuaMessageType::Error, "Cannot register class %s", unknown_class);
		return;
	}
	add(new CObjectItemScript(
#ifndef NO_XR_GAME
...
void CObjectFactory::register_script() const
{
	...
	luabind::module(::ScriptEngine->lua())[instance];
}
